...
# Versão: 2.0 - Atualizada com sistema if ($pedidosResult.Success) {
    $pedidos = $pedidosResult.Data.dados
    Write-Host "✅ Pedidos listados com sucesso!" -ForegroundColor Greenmonitoramento
...
$healthResult = Measure-RequestTime {
    Invoke-ApiRequest -Url "$BASE_URL/admin/metrics/health"
}
...
Write-Host "   - Acesse: $BASE_URL/admin/metrics/dashboard" -ForegroundColor White
